...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    let crossProduct = (x1 * x2) + (y1 * y2);

    let magnitudeFir = Math.sqrt(Math.pow(x1, 2) + Math.pow(y1, 2));
    let magnitudeSec = Math.sqrt(Math.pow(x2, 2) + Math.pow(y2, 2));

    return Math.acos(crossProduct / (magnitudeFir * magnitudeSec));
}
